...
* ``-W <seconds>`` bounds the per-packet wait inside the binary;
...
The output parser handles both GNU (Linux) and BSD (macOS/FreeBSD) formats
with simple, conservative pattern matching.
"""
...
def _parse_output(host: str, output: str, sent: int) -> dict:
    """Parse GNU/BSD ping stdout+stderr into a result dict."""
